...
- 🎯 Entry points to execute your program using `python -m <fh>` or `$ fh` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/adekunleba/fh/branch/main/graph/badge.svg?token=fh_token_here)](https://codecov.io/gh/adekunleba/fh)
[![CI](https://github.com/adekunleba/fh/actions/workflows/main.yml/badge.svg)](https://github.com/adekunleba/fh/actions/workflows/main.yml)

Awesome fh created by adekunleba

## Install

from source
```bash
git clone https://github.com/adekunleba/fh fh
cd fh
make install
```
...
```bash
❯ fh create-user --help
Usage: fh create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fh create-user admin admin --superuser
```
...
Run with `fh run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fh create-user`.
...
-----------------------------------------------------
fh/__init__.py              4      0   100%
fh/app.py                  16      1    94%
fh/cli.py                  21      0   100%
fh/config.py                5      0   100%
fh/db.py                   10      0   100%
fh/models/__init__.py       0      0   100%
fh/models/content.py       47      1    98%
fh/routes/__init__.py      11      0   100%
fh/routes/content.py       52     25    52%
fh/routes/security.py      15      1    93%
fh/routes/user.py          52     26    50%
fh/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fh/default.toml.
